...
class ChatbotController extends Controller
{
    ...
    {
        $validator = Validator::make($request->all(), ['message' => ['required', 'string', 'max:1500'], 'page' => ['nullable', 'string', 'max:500']]);
        if ($validator->fails()) {
        ...
        if (!$match) {
            ...
            try {
                $systemPrompt = implode("\n\n", [
                    ...
                    'Reply concisely using plain text (you may use **bold**, *italic*, lists, and [links](url) for formatting). Help with anime, manga, fandoms, and questions about FanHub Plus itself (navigation, features, how to use the site). Avoid spoilers unless requested. Do not invent site features, policies, release dates, or account access that do not exist. If unsure, say so.',
                    ($page ? 'The user is currently on this page: '.$page.'. Use this to give more relevant answers (e.g. if they are on an anime page, reference that anime).' : ''),
