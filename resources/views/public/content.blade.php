...
<article class="fandom-reading fandom-page--{{ $content->category?->slug ?? 'all' }}">
    <header class="fandom-reading__header">
        <a class="fandom-back" href="{{ route('public.explore', array_filter(['category' => $content->category?->slug])) }}">← Explore {{ $content->category?->name ?? 'content' }}</a>
        <p class="fandom-eyebrow">{{ $content->category?->name ?? 'Content' }} / {{ ucfirst($content->type ?? 'article') }} @if($content->is_featured) / EDITOR'S PICK @endif</p>
        <h1>{{ $content->title }}</h1>
    ...
    @if($related->isNotEmpty())<section class="fandom-reading__related" aria-labelledby="related-title"><div class="fandom-section-title"><div><p class="fandom-eyebrow">KEEP EXPLORING</p><h2 id="related-title">One more <em>discovery.</em></h2></div></div><div class="fandom-content-grid">@foreach($related as $item)<x-fandom-content-card :content="$item" />@endforeach</div></section>@endif
