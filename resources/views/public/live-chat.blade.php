...
<title>Live Chat · FanHub Plus</title>
<script>try { document.documentElement.dataset.theme = localStorage.getItem('fanhub-theme') === 'light' ? 'light' : 'dark'; } catch (e) {}</script>
<link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@500;600;700&family=Saira:wght@400;500;600&display=swap" rel="stylesheet" />
@vite(['resources/css/app.scss', 'resources/js/app.js'])
</head>
<body class="fh-live" data-live-chat>

{{-- Background glow --}}
<div class="fh-live__bg-glow" aria-hidden="true"></div>

{{-- Back button --}}
<a href="{{ route('home') }}" class="fh-live__back">&larr; Back to home</a>
...
(function() {
    ...
    const addMessage = (msg) => {
        ...
        div.innerHTML = '<div class="fh-live__msg-avatar">' + msg.avatar + '</div><div class="fh-live__msg-body"><div class="fh-live__msg-meta"><strong>' + msg.user + '</strong><time>' + msg.time + '</time></div><div class="fh-live__msg-bubble">' + msg.message.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</div></div>';
        messagesEl.append(div);
        requestAnimationFrame(scroll);
    };
    const poll = async () => {
        try {
            const res = await fetch('{{ route("public.live-chat.fetch") }}?after=' + lastId, { headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(8000) });
            if (!res.ok) return;
            const data = await res.json();
            data.messages.forEach(msg => { if (msg.id > lastId) { addMessage(msg); lastId = msg.id; } });
        } catch {}
    };
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch('{{ route("public.live-chat.send") }}', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify({ message: text }), signal: AbortSignal.timeout(10000) });
                const data = await res.json();
            ...
            finally { input.disabled = false; chat.querySelector('[data-live-send]').disabled = false; input.focus(); }
        });
    }
})();
</script>
