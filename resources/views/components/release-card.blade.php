...
<article class="release-card home-tone--{{ $release['slug'] }}" data-release-card data-stagger-item>
    <a href="{{ $release['url'] }}" aria-labelledby="release-title-{{ $release['id'] }}">
        <div class="release-poster"><img src="{{ $release['image'] ?? asset(config('homepage.images.upcoming')) }}" width="1382" height="2048" alt="" loading="lazy" decoding="async"><x-fandom-badge :slug="$release['slug']" :label="$release['category']" /></div>
